...
function parseStatement() {
  ...
  if (Keyword === token.type) {
    switch (token.value) {
      ...
      case 'match':    next(); return parseMatchStatement();
...
function parseMatchStatementLine() {
  if (consume('(')) {
    const parameters = [];
...
function parseMatchStatement() {
  ...
  if ('end' !== token.value) {
    ...
    var expression = parseMatchStatementLine();
    if (null != expression && !defaultExpression) body.push(expression);
    ...
    while (true) {
      ...
      if (!defaultExpression && !expression.isDefaultExpression) body.push(expression);
  ...
  return finishNode(b.matchStatement(identifier, body));
}
...
function parsePrimaryExpression() {
  ...
  } else if (Keyword === type && 'match' === value) {
    pushLocation(marker);
    next();
